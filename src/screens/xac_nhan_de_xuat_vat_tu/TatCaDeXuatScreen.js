...
import * as xndxvttAction from "../../redux/xac_nhan_de_xuat_vat_tu/xndxvt_actions";
import ContentLoader, {Rect} from "react-content-loader/native";
import DanhSachDeXuatComponent from "./DanhSachDeXuatComponent";
...
class TatCaDeXuatScreen extends Component {
    constructor(props) {
        super(props);
    ...
    _makeRemoteRequest = (cb) => {
        ...
        this.props.xndxvttAction.getListAllProposal(this.dataFilter, (error, data) => {
            let dataWHAllTemp = [...this.state.dataWHAll];
            if (error) {
                ...
                dataWHAllTemp = dataWHAllTemp && data.rows ? dataWHAllTemp.concat(data.rows) : data.rows;
                this.setState({
                    dataWHAll: dataWHAllTemp,
                    isSubmit: true
                }, () => {
                    cb && cb(data.total - dataWHAllTemp.length);
    ...
    render() {
        let {valueDateFrom, valueDateTo, dataWHAll} = this.state;
        const {getListAllProposal} = this.props;
        const getList = getListAllProposal && getListAllProposal.rows ? getListAllProposal.rows : [];
        return (
            <View style={styles.container}>
                ...
                <CListView
                    ref="CListView"
                    data={getList ? getList : dataWHAll}
                    extraData={getList ? getList : dataWHAll}
...
export default connect(state => ({
        getListAllProposal: state.xndxvt.getListAllProposal
    }), (dispatch) => ({
        navigationActions: bindActionCreators(navigationActions, dispatch),
        xndxvttAction: bindActionCreators(xndxvttAction, dispatch),
    })
)(TatCaDeXuatScreen);
...
const styles = {
    ...
    txtTitle: {
        ...
        lineHeight: Math.floor(Scale(25, true))
    },
    ...
    txtAll: {
        fontSize: Scale(16, true),
        lineHeight: Math.floor(Scale(60, true)),
        fontFamily: Config.getFont('Muli-Bold')
    },
    ...
    titleNew: {
        paddingHorizontal: Scale(16),
        lineHeight: Math.floor(Scale(36, true)),
        fontFamily: Config.getFont('Muli-Bold'),
        fontSize: Scale(14, true),
    },
