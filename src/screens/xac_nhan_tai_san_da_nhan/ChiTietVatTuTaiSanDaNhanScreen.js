...
class ChiTietVatTuTaiSanDaNhanScreen extends Component {
    constructor(props) {
        super(props);
    ...
    componentDidMount() {
        ...
        this.setState({
            dataInventory: getDetailConfirmReceived ? getDetailConfirmReceived : [],
        });
    }
    ...
    onChangeValue = (number, idx) => {
        ...
        if (this.dataInventoryTemp.Inventory[idx]) {
            this.dataInventoryTemp.Inventory[idx].ReceivedQuantity = number ? parseInt(number) : 0;
        }
        this.setState({
...
)(ChiTietVatTuTaiSanDaNhanScreen);
