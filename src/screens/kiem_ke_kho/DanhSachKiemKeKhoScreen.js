            ...
            <View style={styles.componentContainer}>
                ...
                >
                    <View style={{height: '100%', width: '100%'}}>
                        ...
                        <View style={styles.componentViewContent}>
                            <Text
                                style={styles.componentTextContent}>{Config.lang("PM_Kho")}</Text>
                            <View style={styles.viewDashContent}>
                            </View>
...
class DanhSachKiemKeKhoScreen extends Component {
    constructor(props) {
        super(props);
    ...
    onLink = (data) => {
        this.props.navigationActions.changeScreen('ChiTietPhieuKiemKeTabScreen',{VoucherID:data.VoucherID, reload: this.onReLoad});
    };
    ...
    onReLoad = () => {
        ...
        this.props.kiemkekhoAction.getListInventoryKKK(params, (error, data) => {
            if (error) {
    ...
    _makeRemoteRequest = (cb) => {
        ...
        this.props.kiemkekhoAction.getListInventoryKKK(this.dataFilter, (error, data) => {
            // console.log("getListInventoryKKK getListInventoryKKK", error, data);
            let dataDSKKKTemp = [...this.state.dataDSKKK];
            if (error) {
                Config.alertMess({code: '', message: error.message});
                this.setState({
                    dataDSKKK: [],
                ...
                dataDSKKKTemp = dataDSKKKTemp && data.rows ? dataDSKKKTemp.concat(data.rows) : data.rows;
                this.setState({
                    dataDSKKK: dataDSKKKTemp,
                    isSubmit: true
                }, () => {
                    cb && cb(data.total - dataDSKKKTemp.length);
    ...
    render() {
        let {dataDSKKK, valueDateFrom, valueDateTo} = this.state;
        const {getListInventoryKKK} = this.props;
        // console.log("getListInventoryKKK getListInventoryKKK", getListInventoryKKK);
        // console.log("dataDSKKK dataDSKKK", dataDSKKK);
        return (
            <View style={styles.container}>
                ...
                <CListView
                    ref="CListView"
                    data={getListInventoryKKK && getListInventoryKKK.rows ? getListInventoryKKK.rows : dataDSKKK}
                    extraData={getListInventoryKKK && getListInventoryKKK.rows ? getListInventoryKKK.rows : dataDSKKK}
...
export default connect(state => ({
        getListInventoryKKK: state.kiemkekho.getListInventoryKKK,
    }),
    (dispatch) => ({
        kiemkekhoAction: bindActionCreators(kiemkekhoAction, dispatch),
        navigationActions: bindActionCreators(navigationActions, dispatch),
    })
)(DanhSachKiemKeKhoScreen);
